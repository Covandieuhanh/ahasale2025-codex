...
public partial class gianhang_ho_so_tieu_dung : Page
{
    private const int WalletType = 1;
    ...
    {
        Response.Redirect("/gianhang/ho-so-tieu-dung.aspx", false);
        Context.ApplicationInstance.CompleteRequest();
    }
    ...
    private void BindPage(RootAccount_cl.RootAccountInfo info)
    {
        ...
        using (dbDataContext db = new dbDataContext())
        {
            ...
            lit_balance_a.Text = history.Balance.TieuDung.ToString("#,##0.##") + " A";
            lit_balance_vnd.Text = (history.Balance.TieuDung * VND_PER_A).ToString("#,##0") + " đ";
    ...
    protected string CleanNote(object noteRaw)
    {
        ...
        note = note.Replace(GianHangLedger_cl.TagCreditSeller, string.Empty)
                   ...
                   .Trim();
        return note == string.Empty ? "Giao dịch hồ sơ quyền tiêu dùng gian hàng" : Server.HtmlEncode(note);
    }
    ...
    {
        string url = "/gianhang/ho-so-tieu-dung.aspx";
