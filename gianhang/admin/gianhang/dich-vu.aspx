<%@ Page Title="Dịch vụ /gianhang" Language="C#" MasterPageFile="~/gianhang/admin/mp-admin.master" AutoEventWireup="true" CodeFile="dich-vu.aspx.cs" Inherits="gianhang_admin_gianhang_dich_vu" %>

<asp:Content ID="ContentHead" ContentPlaceHolderID="head" runat="server">
    <style>
        .gh-catalog-shell{display:grid;gap:18px}
        .gh-catalog-hero,.gh-catalog-card{border:1px solid #d9efe0;border-radius:22px;background:#fff;box-shadow:0 18px 34px rgba(15,23,42,.06)}
        .gh-catalog-hero{padding:22px;background:radial-gradient(760px 240px at 0% 0%, rgba(34,197,94,.10), transparent 60%),linear-gradient(180deg,#f7fff8 0%,#fff 100%);display:flex;justify-content:space-between;align-items:flex-start;gap:18px;flex-wrap:wrap}
        .gh-catalog-kicker{display:inline-flex;align-items:center;min-height:30px;padding:0 12px;border-radius:999px;background:#eefbf0;border:1px solid #cfead3;color:#166534;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.03em}
        .gh-catalog-title{margin:10px 0 6px;color:#16331d;font-size:30px;line-height:1.12;font-weight:900}
        ...
        .gh-catalog-btn--primary{background:linear-gradient(135deg,#2f7a35,#52a35b);color:#fff!important;box-shadow:0 14px 28px rgba(47,122,53,.16)}
        .gh-catalog-btn--soft{background:#fff;color:#16331d!important;border-color:#d9efe0}
        ...
        .gh-catalog-input:focus{border-color:#52a35b;box-shadow:0 0 0 3px rgba(82,163,91,.12)}
        .gh-catalog-submit,.gh-catalog-reset{min-height:42px;border-radius:14px;padding:0 16px;font-size:13px;font-weight:800}
        .gh-catalog-submit{border:0;background:linear-gradient(135deg,#2f7a35,#52a35b);color:#fff}
        ...
        .gh-catalog-link--primary{border-color:#cfead3;background:#eefbf0}
    ...
    <div class="gh-catalog-shell">
        <section class="gh-catalog-hero">
            <div>
                ...
                <h1 class="gh-catalog-title">Dịch vụ /gianhang</h1>
                <p class="gh-catalog-sub">Đây là lớp dịch vụ native của workspace /gianhang. Mình dùng nó để kiểm tra hiển thị public, ưu đãi dịch vụ, trạng thái bridge admin và điều hướng sang booking/storefront mượt hơn.</p>
            </div>
            <div class="gh-catalog-actions">
                ...
                <a class="gh-catalog-btn gh-catalog-btn--soft" href="<%=CompanionUrl %>">Sản phẩm /gianhang</a>
        ...
        <section class="gh-catalog-card">
            <div class="gh-catalog-filters">
                <div class="gh-catalog-field">
                    <label for="<%= txt_search.ClientID %>">Tìm dịch vụ</label>
        ...
        <div class="gh-catalog-summary">
            <div class="gh-catalog-summary-card"><small>Tổng dịch vụ</small><strong><%=TotalCount.ToString("#,##0") %></strong><span>Tổng dịch vụ native đang được storefront quản lý.</span></div>
            <div class="gh-catalog-summary-card"><small>Đang hiển thị</small><strong><%=VisibleCount.ToString("#,##0") %></strong><span>Dịch vụ đang mở cho khách đặt lịch hoặc xem công khai.</span></div>
            <div class="gh-catalog-summary-card"><small>Đang ẩn</small><strong><%=HiddenCount.ToString("#,##0") %></strong><span>Dịch vụ đã tắt hoặc đang ẩn khỏi mặt public.</span></div>
            <div class="gh-catalog-summary-card"><small>Đã bridge</small><strong><%=MirroredCount.ToString("#,##0") %></strong><span>Dịch vụ đã có bản ghi bridge sang quản lý admin.</span></div>
            <div class="gh-catalog-summary-card"><small>Đang ưu đãi</small><strong><%=DiscountedCount.ToString("#,##0") %></strong><span>Dịch vụ đang đặt ưu đãi để hỗ trợ chốt sale/booking.</span></div>
        </div>
        ...
        <section class="gh-catalog-card">
            <div class="gh-catalog-table-wrap">
                <table class="gh-catalog-table">
                    <thead>
                        <tr>
                            <th style="min-width:320px;">Dịch vụ</th>
                    ...
                    <tbody>
                        ...
                        <asp:PlaceHolder ID="ph_empty" runat="server" Visible="false">
                            <tr><td colspan="8" class="gh-catalog-empty">Chưa có dịch vụ native nào khớp với bộ lọc hiện tại.</td></tr>
                        </asp:PlaceHolder>
                    </tbody>
                </table>
            </div>
