<%@ WebHandler Language="C#" Class="GianHangWarehouseLookupHandler" %>
...
public class GianHangWarehouseLookupHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
        ...
        try
        {
            using (dbDataContext db = new dbDataContext())
            {
                ...
                var query = db.web_post_tables
                    .Where(p => p.id_chinhanh == branchId && p.phanloai == "ctsp" && p.bin == false && p.name.Contains(keywordRaw))
                    .OrderBy(p => p.name)
                    .Take(20)
                    .ToList();
                ...
                LookupItem item = match == null
                    ...
                    : new LookupItem
                    {
                        ...
                        PriceText = ((match.giavon_sanpham ?? 0).ToString("#,##0")),
