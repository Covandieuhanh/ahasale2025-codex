...
var TOM_SELECT_CSS_ID = "aha-tomselect-css";
var TOM_SELECT_JS_ID = "aha-tomselect-js";
var TOM_SELECT_CSS_HREF = "https://cdn.jsdelivr.net/npm/tom-select/dist/css/tom-select.css";
var TOM_SELECT_JS_HREF = "https://cdn.jsdelivr.net/npm/tom-select/dist/js/tom-select.complete.min.js";
var tomSelectLoading = false;
var tomSelectReadyQueue = [];

function flushTomSelectReadyQueue(ok) {
    var queue = tomSelectReadyQueue.slice(0);
    tomSelectReadyQueue = [];
    tomSelectLoading = false;
    queue.forEach(function (fn) {
        try { fn(ok); } catch (e) { }
    });
}

function ensureTomSelectAssets(callback) {
    ...
    tomSelectReadyQueue.push(callback);
    if (tomSelectLoading) return;
    tomSelectLoading = true;

    if (!document.getElementById(TOM_SELECT_CSS_ID)) {
        var css = document.createElement("link");
        css.id = TOM_SELECT_CSS_ID;
        css.rel = "stylesheet";
        css.href = TOM_SELECT_CSS_HREF;
        document.head.appendChild(css);
    }

    var existingScript = document.getElementById(TOM_SELECT_JS_ID);
    if (existingScript) {
        existingScript.addEventListener("load", function () { flushTomSelectReadyQueue(true); });
        existingScript.addEventListener("error", function () { flushTomSelectReadyQueue(false); });
        return;
    }

    var script = document.createElement("script");
    script.id = TOM_SELECT_JS_ID;
    script.src = TOM_SELECT_JS_HREF;
    script.defer = true;
    script.onload = function () { flushTomSelectReadyQueue(true); };
    script.onerror = function () { flushTomSelectReadyQueue(false); };
...
function initAllTomSelect() {
    ...
    if (typeof TomSelect === "undefined") {
        ensureTomSelectAssets(function (ok) {
            if (ok) initAllTomSelect();
        });
        return;
    }
...
(function () {
    ...
    function lockBodyScroll() {
        ...
        var scrollbarWidth = Math.max(0, window.innerWidth - (docEl ? docEl.clientWidth : window.innerWidth));
