            ...
            else // nếu tài khoản tồn tại
            {
                ...
                if (_mk != (_ob.matkhau ?? "")) // so sánh với mật khẩu được giải mã từ Cookie, nếu khác nhau
                {
                    ...
                    if (_chuyentrang == true)
                    {
                        if (shopFlow)
                        {
                            HttpContext.Current.Session["thongbao_shop"] = thongbao_class.metro_dialog_onload("Thông báo", "Mật khẩu đã được thay đổi. <br/>Vui lòng đăng nhập lại.", "false", "false", "OK", "alert", "");
                            HttpContext.Current.Response.Redirect("/shop/login.aspx");
                        }
                        else
                        {
                ...
                else // tiếp tục xử lý
                {
                    if (_ob.block == true) // nếu tài khoản này bị khóa
                    {
                        ...
                        if (_chuyentrang == true)
                        {
                            if (shopFlow)
                            {
                                HttpContext.Current.Session["thongbao_shop"] = thongbao_class.metro_dialog_onload("Thông báo", "Tài khoản shop đã bị khóa.", "false", "false", "OK", "alert", "");
                                HttpContext.Current.Response.Redirect("/shop/login.aspx");
                            }
                            else
                            {
                    ...
                    else
                    {
                        if (_ob.hansudung != null && AhaTime_cl.Now > _ob.hansudung.Value) // nếu có hạn sử dụng và hết hạn
                        {
                            ...
                            if (_chuyentrang == true)
                            {
                                if (shopFlow)
                                {
                                    HttpContext.Current.Session["thongbao_shop"] = thongbao_class.metro_dialog_onload("Thông báo", "Tài khoản shop của bạn đã hết hạn sử dụng.", "false", "false", "OK", "alert", "");
                                    HttpContext.Current.Response.Redirect("/shop/login.aspx");
                                }
                                else
                                {
                        ...
                        else // kiểm tra phạm vi đăng nhập
                        {
                            ...
                            if (!canLoginHome && !canLoginShopBridge)
                            {
                                ...
                                if (_chuyentrang == true)
                                {
                                    if (shopFlow)
                                    {
                                        HttpContext.Current.Session["thongbao_shop"] = thongbao_class.metro_dialog_onload("Thông báo", "Tài khoản này không được phép dùng module này.", "false", "false", "OK", "alert", "");
    ...
    public static void check_login_shop(string _quyen1, string _quyen2, bool _chuyentrang)
    {
        using (dbDataContext db = new dbDataContext())
        {
            ...
            if (!taikhoan_cl.exist_taikhoan(_tk))
            {
                ...
                if (_chuyentrang)
                {
                    HttpContext.Current.Session["thongbao_shop"] = thongbao_class.metro_notifi_onload("Thông báo", "Vui lòng đăng nhập để tiếp tục.", "1000", "warning");
                    HttpContext.Current.Response.Redirect("/shop/login.aspx");
                }
                return;
            }
            ...
            if (_ob.block == true)
            {
                ...
                if (_chuyentrang)
                {
                    HttpContext.Current.Session["thongbao_shop"] = thongbao_class.metro_dialog_onload("Thông báo", "Tài khoản đã bị khóa.", "false", "false", "OK", "alert", "");
                    HttpContext.Current.Response.Redirect("/shop/login.aspx");
                }
                return;
            }
            ...
            if (_ob.hansudung != null && AhaTime_cl.Now > _ob.hansudung.Value)
            {
                ...
                if (_chuyentrang)
                {
                    HttpContext.Current.Session["thongbao_shop"] = thongbao_class.metro_dialog_onload("Thông báo", "Tài khoản của bạn đã hết hạn sử dụng.", "false", "false", "OK", "alert", "");
                    HttpContext.Current.Response.Redirect("/shop/login.aspx");
                }
                return;
            }
            ...
            if (!canLoginShop)
            {
                ...
                if (_chuyentrang)
                {
                    HttpContext.Current.Session["thongbao_shop"] = thongbao_class.metro_dialog_onload("Thông báo", "Tài khoản này chỉ được phép đăng nhập ở " + targetPortal + ".", "false", "false", "OK", "alert", "");
                    HttpContext.Current.Response.Redirect("/shop/login.aspx");
                }
                return;
            }
            ...
            else
            {
                if (_chuyentrang)
                {
                    HttpContext.Current.Session["thongbao_shop"] = thongbao_class.metro_dialog_onload("Thông báo", "Bạn không đủ quyền thực hiện thao tác vừa rồi.", "false", "false", "OK", "alert", "");
                    HttpContext.Current.Response.Redirect("/shop/default.aspx");
                }
            }
        }
