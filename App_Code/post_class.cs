...
public class post_class
{
    ...
    {
        var q = db.web_post_tables.Where(p => p.id_chinhanh == CurrentChiNhanhId && p.bin == false).ToList();
        return q;
    }
    #endregion
    ...
    public void del(string _id)
    {
        ...
        if (q.Count() != 0)
        {
            ...
            AhaShineHomeSync_cl.RemovePost(db, _po_tb);
    ...
    public void change_status_bin(string _id, bool _stt)//thay đổi trạng thái true false của thùng rác, true là đã xóa, false là chưa xóa
    {
        ...
        _tb_oj.bin = _stt;
        db.SubmitChanges();
        AhaShineHomeSync_cl.SyncPost(db, _tb_oj);
    }
    ...
    public int return_index_dv(string _id)
    {
        ...
        foreach (var t in db.web_post_tables.Where(p => p.phanloai == "ctdv" && p.id_chinhanh == CurrentChiNhanhId))
        {
    ...
    {
        int i = 1;      //=1 khi có thêm iteam "Chọn" luôn nằm đầu tiên mà chỉ số index bắt đầu =0                                    
        foreach (var t in db.web_post_tables.Where(p => p.phanloai == "ctdv" && p.id_nganh == CurrentNganhId && p.id_chinhanh == CurrentChiNhanhId))
        {
            if (_id == t.id.ToString())
                return i;
            i = i + 1;
