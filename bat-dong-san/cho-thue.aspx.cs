...
public partial class bat_dong_san_cho_thue : BatDongSanPageBase
{
    ...
    private void BindFilters()
    {
        ...
        BatDongSanListingFilterHelper_cl.PopulateRentFilters(selectedProvince, selectedDistrict, ddlProvince, ddlDistrict, ddlPropertyType, ddlBedrooms, ddlFurnishing, ddlProject, ddlPrice, ddlArea, ddlSort);
    }

    private void ApplyQueryToControls()
    {
        BatDongSanListingFilterHelper_cl.ApplyQueryToControls(Request, ddlProvince, ddlDistrict, ddlPropertyType, ddlBedrooms, null, ddlFurnishing, ddlProject, ddlPrice, ddlArea, ddlSort, txtKeyword);
    }

    private void BindList()
    {
        BatDongSanService_cl.ListingQuery query = BatDongSanListingFilterHelper_cl.BuildListingQuery("rent", ddlProvince, ddlDistrict, ddlPropertyType, ddlBedrooms, null, ddlFurnishing, ddlProject, ddlPrice, ddlArea, ddlSort, txtKeyword);
        ...
        rptRentListings.DataSource = Listings;
        rptRentListings.DataBind();
    ...
    {
        string url = BatDongSanListingFilterHelper_cl.BuildFilterUrl("/bat-dong-san/cho-thue.aspx", ddlProvince, ddlDistrict, ddlPropertyType, ddlBedrooms, null, ddlFurnishing, ddlProject, ddlPrice, ddlArea, ddlSort, txtKeyword);
    ...
    protected string RenderIntentHighlights(object itemObj)
    {
        ...
        if (item.PriceText != "" && item.PriceText != "Liên hệ")
            parts.Add("<span class='bds-intent-pill'>Giá " + Server.HtmlEncode(item.PriceText) + "</span>");

        if (item.AreaValue > 0 && (propertyType == "business-premises" || propertyType == "warehouse" || propertyType == "office"))
    ...
    protected string RenderListingSummary(object itemObj)
    {
        ...
        if (summary == "")
            summary = item.IsLinked ? "Xem tin gốc để đọc đầy đủ mô tả và điều kiện thuê." : "Xem chi tiết để đọc thêm thông tin cho thuê.";
    ...
    private void AddRemovableChip(List<string> chips, string text, string key)
    {
        ...
        string url = BatDongSanListingFilterHelper_cl.BuildFilterRemovalUrl("/bat-dong-san/cho-thue.aspx", Request, normalizedKey);
