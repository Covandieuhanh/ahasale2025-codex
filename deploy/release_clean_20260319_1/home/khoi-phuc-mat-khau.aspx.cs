    ...
    protected void btnSendOtp_Click(object sender, EventArgs e)
    {
        using (dbDataContext db = new dbDataContext())
        {
            ...
            if (requestId <= 0)
            {
                string manualOtp;
                string manualError;
                if (!HomeOtp_cl.CreateManualOtp(db, phone, q.taikhoan, HomeOtp_cl.TypePassword, out requestId, out manualOtp, out manualError))
                {
            ...
            else
            {
                ...
                string lookupError;
                if (!HomeOtp_cl.TryGetOtpInfo(db, requestId, HomeOtp_cl.TypePassword, out info, out lookupError))
                {
                    string manualOtp;
                    string manualError;
                    if (!HomeOtp_cl.CreateManualOtp(db, phone, q.taikhoan, HomeOtp_cl.TypePassword, out requestId, out manualOtp, out manualError))
