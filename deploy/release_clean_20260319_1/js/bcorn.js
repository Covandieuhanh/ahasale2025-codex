...
/*tự nhảy dấu chấm khi nhập số tiền trên textbox onChange="format_sotien(this);"*/
function format_sotien(a) { a.value = a.value.replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1."); }
...
//heartBeat
function heartBeat() { $.get("/KeepAlive.ashx?", function (data) { }); } $(function () { setInterval("heartBeat()", 1000 * 30); });
