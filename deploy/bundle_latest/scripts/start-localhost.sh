...
echo "Current app DB mode:"
"$ROOT_DIR/scripts/show-db-mode.sh"
echo
...
if [ -f docker-compose.override.local.yml ]; then
  COMPOSE_ARGS+=(-f docker-compose.override.local.yml)
fi

docker compose "${COMPOSE_ARGS[@]}" up -d --build db init-db web
docker compose "${COMPOSE_ARGS[@]}" stop admin >/dev/null 2>&1 || true
...
echo "Waiting for https://ahasale.local to be ready..."
for _ in $(seq 1 30); do
  code="$(curl -k -s -o /dev/null -w "%{http_code}" --max-time 5 --resolve ahasale.local:443:127.0.0.1 https://ahasale.local || true)"
  if [ "$code" = "200" ]; then
    break
  fi
  sleep 2
...
echo "Waiting for https://ahasale.local/admin/login.aspx to be ready..."
for _ in $(seq 1 30); do
  code="$(curl -k -s -o /dev/null -w "%{http_code}" --max-time 5 --resolve ahasale.local:443:127.0.0.1 https://ahasale.local/admin/login.aspx || true)"
  if [ "$code" = "200" ]; then
    break
  fi
  sleep 2
...
echo "Open: https://ahasale.local"
echo "Admin portal: https://ahasale.local/admin/login.aspx"
echo "Admin account: admin / 123"
echo "DB (optional): localhost,11433 / ${LOCAL_DB_NAME} (sa / AhaSaleLocal#2026)"
echo "Runtime app DB mode:"
"$ROOT_DIR/scripts/show-db-mode.sh"
