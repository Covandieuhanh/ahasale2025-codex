...
public partial class home_don_chi_tiet : System.Web.UI.Page
{
    ...
    private string BuildReviewUrl(object nameEnObj, object idObj)
    {
        string baseUrl = BuildProductUrl(nameEnObj, idObj);
        if (string.IsNullOrEmpty(baseUrl) || baseUrl == "#")
            return "#";
        if (baseUrl.Contains("?"))
            return baseUrl + "&review=1#review-section";
        return baseUrl + "?review=1#review-section";
    }
    ...
    private void LoadOrder()
    {
        ...
        using (dbDataContext db = new dbDataContext())
        {
            ...
            var rawDetails = (from ct in db.DonHang_ChiTiet_tbs.Where(p => p.id_donhang == orderId)
            ...
            var itemIds = rawDetails
                .Select(p => (p.idsp ?? "").Trim())
                .Where(p => p != "")
                .Distinct()
                .ToList();

            var reviewedSet = new System.Collections.Generic.HashSet<string>(StringComparer.OrdinalIgnoreCase);
            bool allowReview = mode == "buy" && isCompleted && itemIds.Count > 0;
            if (allowReview)
            {
                reviewedSet = new System.Collections.Generic.HashSet<string>(
                    db.DanhGiaBaiViets
                        .Where(dg => dg.TaiKhoanDanhGia == currentAccount && itemIds.Contains(dg.idBaiViet))
            ...
            var details = rawDetails.Select(p => new
            {
                p.id,
                p.idsp,
                p.name,
                ...
                p.thanhtien,
                p.PhanTramUuDai,
                show_review = allowReview && !reviewedSet.Contains((p.idsp ?? "").Trim()),
                review_url = BuildReviewUrl(p.name_en, p.idsp),
                show_reviewed = allowReview && reviewedSet.Contains((p.idsp ?? "").Trim())
            }).ToList();
