...
public partial class shop_nang_cap_level2 : System.Web.UI.Page
{
    private const string NoticeSessionKey = "shop_level2_notice";
        ...
        using (dbDataContext db = new dbDataContext())
        {
            ...
            if (!ShopLevel2Request_cl.TryCreateRequest(db, shopAccount, out message))
            {
                Session[NoticeSessionKey] = string.IsNullOrWhiteSpace(message)
                    ? "Không thể gửi yêu cầu nâng cấp Level 2."
                    : message;
                Response.Redirect("/shop/nang-cap-level2.aspx", false);
                Context.ApplicationInstance.CompleteRequest();
                return;
            }
    ...
    protected void but_reset_owner_admin_Click(object sender, EventArgs e)
    {
        ...
        using (dbDataContext db = new dbDataContext())
        {
            ...
            {
                Session[NoticeSessionKey] = "Shop này chưa bật Level 2 nên chưa thể đặt lại mật khẩu owner admin.";
                Response.Redirect("/shop/nang-cap-level2.aspx", false);
                Context.ApplicationInstance.CompleteRequest();
                return;
            }
            ...
            {
                Session[NoticeSessionKey] = "Chưa tìm thấy owner admin để đặt lại mật khẩu.";
                Response.Redirect("/shop/nang-cap-level2.aspx", false);
                Context.ApplicationInstance.CompleteRequest();
                return;
            }
        ...
        Response.Redirect("/gianhang/admin/quen-mat-khau/dat-lai-mat-khau.aspx?from=shop", false);
        Context.ApplicationInstance.CompleteRequest();
    }
    ...
    private void BindPage()
    {
        ...
        using (dbDataContext db = new dbDataContext())
        {
            ...
            lb_shop_level.Text = ShopLevel_cl.ResolveLevelLabel(db, shopAccount);
            ...
            lb_admin_account_info.Text = "Owner admin mặc định dùng username " + HttpUtility.HtmlEncode(ShopLevel_cl.ResolveOwnerAdminUsername(shopAccount)) + ".";
            lb_password_info.Text = ownerAdmin == null
                ? "Khi được duyệt Level 2, owner admin mới sẽ có mật khẩu mặc định 123456."
                : "Nếu owner admin đã tồn tại, dùng mật khẩu hiện có hoặc reset lại từ màn đăng nhập admin.";
