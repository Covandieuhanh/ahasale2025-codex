...
- Da them che do chuyen DB runtime co chu dich: `local` va `shared-online`
...
## Che do DB runtime
Mac dinh repo nay chay `local`:
- App runtime: `db:1433 / ahasale_local`
- Docker SQL local: `localhost:11433 / ahasale_local`

Kiem tra mode hien tai:
- `./scripts/show-db-mode.sh`

Chuyen ve DB local on dinh:
- `./scripts/use-db-local.sh`

Chuyen app runtime sang DB online dung chung:
- `./scripts/use-db-shared-online.sh`

Luu y quan trong:
- `shared-online` chi doi `Web.config` va `Web.local.config` cua app runtime.
- Cac script bao tri local van tro ve DB Docker `ahasale_local` de tranh ghi/restore nham DB online.
- `shared-online` chi hoat dong neu may local co duong ket noi SQL Server toi `112.78.2.206:1433` (VPN, SSH tunnel, whitelist IP, hoac route noi bo).
- Neu khong co duong ket noi nay, `ahasale.local` se loi ngay o tang ket noi DB.
