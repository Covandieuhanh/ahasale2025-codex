...
public class Helper_DongA_cl
{
    ...
    private static bool? _hasLedgerBehaviorColumn = null;

    private static bool HasLedgerBehaviorColumn(dbDataContext db)
    {
        ...
        try
        {
            int? len = db.ExecuteQuery<int?>("SELECT COL_LENGTH('dbo.LichSu_DongA_tb','KyHieu9HanhVi_1_9')").FirstOrDefault();
            _hasLedgerBehaviorColumn = len.HasValue;
        }
        catch
        {
            _hasLedgerBehaviorColumn = false;
        }
        ...
        int? kyHieuHanhVi = null)
    {
        ...
        {
            db.ExecuteCommand(
                "INSERT INTO dbo.LichSu_DongA_tb (taikhoan, dongA, ngay, CongTru, id_donhang, ghichu, id_rutdiem, LoaiHoSo_Vi) " +
